...
export default function PurchasesPage() {
  ...
  const checkForNewPurchases = async (type: 'saldo' | 'payu') => {
    ...
    try {
      const res = await fetch(`/api/allproducts?page=1&type=${type}`);
      ...
      if (data.purchases && data.purchases.length > 0) {
        ...
        if (latestPurchaseDate > storedLastUpdate) {
          ...
          console.log('Nuevas compras detectadas, actualizando...');
    ...
    if (session) {
      try {
        ...
        if (!forceRefresh) {
          ...
          if (cachedData) {
            console.log('Usando datos en caché para', type);
        ...
        const res = await fetch(`/api/allproducts/numero?page=${page}&type=${type}`);
        const data = await res.json();
        if (data.purchases) {
          const parsedPurchases = data.purchases.map((purchase: Purchase) => {
            try {
              ...
              console.error('Error parsing purchase products:', error);
              return {
                ...purchase,
                products: [{
        ...
        if (!forceRefresh) {
          const cachedData = getFromLocalStorage(type);
          if (cachedData) {
            console.log('Usando datos en caché antiguos debido a error');
            setPurchases(cachedData.purchases);
            setTotalItems(prev => ({
              ...prev,
              [type]: cachedData.pagination.total
  ...
  }, [session, activeTab, currentPagePayu, currentPageSaldo]);
